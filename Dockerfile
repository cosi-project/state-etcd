...
RUN --mount=type=cache,target=/root/.cache/go-build,id=state-etcd/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=state-etcd/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
COPY .golangci.yml .
ENV GOGC=50
RUN --mount=type=cache,target=/root/.cache/go-build,id=state-etcd/root/.cache/go-build --mount=type=cache,target=/root/.cache/golangci-lint,id=state-etcd/root/.cache/golangci-lint,sharing=locked --mount=type=cache,target=/go/pkg,id=state-etcd/go/pkg golangci-lint fmt --config .golangci.yml
RUN --mount=type=cache,target=/root/.cache/go-build,id=state-etcd/root/.cache/go-build --mount=type=cache,target=/root/.cache/golangci-lint,id=state-etcd/root/.cache/golangci-lint,sharing=locked --mount=type=cache,target=/go/pkg,id=state-etcd/go/pkg golangci-lint run --fix --issues-exit-code 0 --config .golangci.yml
...
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=state-etcd/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=state-etcd/go/pkg --mount=type=cache,target=/tmp,id=state-etcd/tmp CGO_ENABLED=1 go test -race ${TESTPKGS}

# runs unit-tests
FROM base AS unit-tests-run
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=state-etcd/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=state-etcd/go/pkg --mount=type=cache,target=/tmp,id=state-etcd/tmp go test -covermode=atomic -coverprofile=coverage.txt -coverpkg=${TESTPKGS} ${TESTPKGS}
