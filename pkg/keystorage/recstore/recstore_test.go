...
func TestRecStore(t *testing.T) {
	testhelpers.WithEtcd(t, func(client *clientv3.Client) {
		...
		err = st.Update(t.Context(), &data{binaryData: hx}, 1)
		errorShouldBe[recstore.NotFoundTag](t, err)

		err = st.Update(t.Context(), &data{binaryData: hx}, 0)
		require.NoError(t, err)

		err = st.Create(t.Context(), &data{binaryData: hx})
		errorShouldBe[recstore.AlreadyExistsTag](t, err)

		err = st.Update(t.Context(), &data{binaryData: hx}, 1)
		require.NoError(t, err)

		err = st.Update(t.Context(), &data{binaryData: hx}, 2)
		require.NoError(t, err)
		...
		err = st.Update(t.Context(), &data{binaryData: []byte("not-expected")}, 3)
		require.NoError(t, err)
